...
// Load and execute additional table creation scripts
async function loadAdditionalTables(pool) {
    try {
        console.log('Loading additional table schemas...');
        
        // Load MSE tables
        const fs = require('fs');
        const path = require('path');
        
        const mseTablesPath = path.join(__dirname, 'create_mse_tables.sql');
        if (fs.existsSync(mseTablesPath)) {
            const mseTablesSQL = fs.readFileSync(mseTablesPath, 'utf8');
            const mseStatements = mseTablesSQL.split(';').filter(stmt => stmt.trim().length > 0);
            
            for (const statement of mseStatements) {
                if (statement.trim()) {
                    try {
                        await pool.query(statement.trim());
                        console.log('✓ MSE table statement executed successfully');
                    } catch (error) {
                        if (!error.message.includes('already exists')) {
                            console.warn('⚠ MSE table statement warning:', error.message);
                        }
                    }
                }
            }
        ...
        // Load Seating Plan tables
        const seatingTablesPath = path.join(__dirname, 'create_seating_plan_tables.sql');
        if (fs.existsSync(seatingTablesPath)) {
            const seatingTablesSQL = fs.readFileSync(seatingTablesPath, 'utf8');
            const seatingStatements = seatingTablesSQL.split(';').filter(stmt => stmt.trim().length > 0);
            
            for (const statement of seatingStatements) {
                if (statement.trim()) {
                    try {
                        await pool.query(statement.trim());
                        console.log('✓ Seating plan table statement executed successfully');
                    } catch (error) {
                        if (!error.message.includes('already exists')) {
                            console.warn('⚠ Seating plan table statement warning:', error.message);
                        }
                    }
                }
            }
        ...
        // Load Exam Timetable tables
        const timetableTablesPath = path.join(__dirname, 'create_exam_timetable_tables.sql');
        if (fs.existsSync(timetableTablesPath)) {
            const timetableTablesSQL = fs.readFileSync(timetableTablesPath, 'utf8');
            const timetableStatements = timetableTablesSQL.split(';').filter(stmt => stmt.trim().length > 0);
            
            for (const statement of timetableStatements) {
                if (statement.trim()) {
                    try {
                        await pool.query(statement.trim());
                        console.log('✓ Exam timetable table statement executed successfully');
                    } catch (error) {
                        if (!error.message.includes('already exists')) {
                            console.warn('⚠ Exam timetable table statement warning:', error.message);
                        }
                    }
                }
            }
        ...
        console.log('✓ Additional tables loaded successfully');
        
    } catch (error) {
        console.error('✗ Error loading additional tables:', error.message);
        throw error;
    }
}

// Modify the main initialization function to include additional tables
const originalInitializeDatabase = module.exports;
module.exports = async function(pool) {
    const result = await originalInitializeDatabase(pool);
    
    // Load additional tables after main initialization
    await loadAdditionalTables(pool);
